...
check('Portal desktop', state.title.includes('FOFO TECH') && state.overflow && state.days === 2, JSON.stringify(state));
...
check('Dia 1 desktop', state.title.includes('Dia 1') && state.overflow && state.activities === 6 && state.resources === 7 && state.external, JSON.stringify(state));
...
await wait(250);
check('Ampliação do recurso', await evaluate(`document.querySelector('.lightbox').open && document.querySelector('.lightbox img').src.includes('arquitetura-m365.svg')`), 'arquitetura Microsoft 365 ampliada');
...
check('Reveal.js e 44 slides', state.ready && state.total === 44 && state.progress && state.number && state.notes, JSON.stringify(state));
...
await evaluate(`Reveal.slide(31)`); await wait(900); await screenshot('10-notebooklm-conceito.png');
await evaluate(`Reveal.slide(33)`); await wait(900); await screenshot('11-notebooklm-funcionalidades.png');
await evaluate(`Reveal.slide(35)`); await wait(900); await screenshot('12-notebooklm-laboratorio.png');
